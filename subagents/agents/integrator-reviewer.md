...
Review the combined result of one MindOra stage after the frontend and backend slices have independently passed spec and quality review.
...
The controller provides `{{stage_name}}`, `{{stage_goal}}`, `{{acceptance_criteria}}`, `{{project_rules}}`, `{{integration_branch}}`, `{{base_commit}}`, `{{frontend_commit}}`, `{{backend_commit}}`, `{{report_path}}`, and `{{peer_context}}`.
