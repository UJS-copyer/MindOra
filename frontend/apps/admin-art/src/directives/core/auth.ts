...
function checkAuthPermission(el: HTMLElement, binding: DirectiveBinding<string>): void {
  ...
  const userHasButton = userStore.getUserInfo.buttons?.includes(binding.value) ?? false
  const hasPermission = isFrontendMode.value
    ? userHasButton
    : authList.length > 0
      ? authList.some((item) => item.authMark === binding.value) && userHasButton
      : userHasButton
