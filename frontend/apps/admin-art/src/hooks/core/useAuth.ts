...
export const useAuth = () => {
  ...
  const hasAuth = (auth: string): boolean => {
    ...
    if (isFrontendMode.value) {
      return info.value?.buttons?.includes(auth) ?? false
    }

    const userHasButton = info.value?.buttons?.includes(auth) ?? false

    // 后端模式下 authList 描述页面按钮清单，buttons 描述当前用户实际拥有的按钮权限。
    if (backendAuthList.length > 0) {
      return backendAuthList.some((item) => item?.authMark === auth) && userHasButton
    }
    return userHasButton
  }
